...
function CheckAttendance() {
  ...
  const fetchEmployeeIds = async () => {
    try {
      const response = await fetch('https://backend-cdu3.onrender.com/employeeRoute/employee-ids');
  ...
  const fetchTimeRecords = async (formattedDate, employeeId) => {
    try {
      const response = await fetch(
        `https://backend-cdu3.onrender.com/timetrackingRoute/get-time-by-employee/${employeeId}?date=${formattedDate}`
      );
